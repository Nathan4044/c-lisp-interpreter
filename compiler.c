...
static uint8_t parseArgs(void)
{
    uint8_t argCount = 0;
    while (!match(TOKEN_RIGHT_PAREN)) {
        if (check(TOKEN_EOF)) {
            error("Unexpected end of file.");
            // dummy value since compilation fails with error
            return 0;
        }
...
static int declareVariable(void)
{
